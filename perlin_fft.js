...
function draw() {
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            ...
            terrain[x][y] = specHeight*map(y, 0, rows, 1.5, .05);
        }
    }
    background(0);
    rotateX(PI/3);
    ...
    for (let y = 0; y < rows-1; y++) {
        ...
        for (let x = 0; x < cols; x++) {
            strColor = map(terrain[x][y], 0, height/4, 0, 360)
            stroke(strColor, 100, 50)
            noFill()
            vertex(x*scl-(width/2), y*scl-(height/2), terrain[x][y]);
            vertex(x*scl-(width/2), (y+1)*scl-(height/2), terrain[x][y+1]);
        }
        endShape()
    }
}